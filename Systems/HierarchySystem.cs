...
public class HierarchySystem : IInitializeSystem {
	...
	void InitEntity() {
		...
		camera.AddComponentBasicMove( true, 0.0015f );
		camera.AddMatRenderSingle( true, 1, null );
		camera.AddMatPara( null, new Dictionary<string, object>(){
			{ "MainTex", "red.png" }
		} );
		camera.AddLightShadowLight( true, new[]{ 0 }, 0, 0.5f, 2f, 0.5f, new Vector3( 0.5f, 0.5f, 0f ), 2f, 15f );

		// var shadowLight21 = _contexts.game.CreateEntity();
		// shadowLight21.AddComponentPosition( -0.3f, 1.5f, 1 );
		// shadowLight21.AddLightShadowLight( true, new[]{ 0 }, 0, 1.2f, 2f, 0.5f, new Vector3( 0.5f, -0.2f, 0f ), 2f, 15f );
