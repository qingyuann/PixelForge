...
public class RendererManager
{
    ...
    {
        _gl = gl;
        // _renderers.Add(new RenderQuad(_gl, new Vector2(0f, 0f), 1f, 1f, 0));
        // _renderers[0].SetUniform("uColor", new Vector3(1, 0, 0));
        //
        // _renderers.Add(new RenderQuad(_gl, new Vector2(-0.01f, 0f), 1f, 1f, 0));
        // _renderers[1].SetUniform("uColor", new Vector3(0, 0, 1));

        var poses = new List<float>();
        for (int i = 0; i < 8; i++)
        {
            poses.Add(RandomTool.Range(-1f, 1f));
            Console.WriteLine(poses[i]);
        }
